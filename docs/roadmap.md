...
- ☑ 3.2 Supplier-document ingestion — content-addressed upload storage, a `RateExtractionProvider` seam, a deterministic pdfplumber grid parser, and a confirm step that is the only path able to create a rate. RBAC split so confirming is its own permission. 78 new tests. Parser coverage is honest and partial: 5 of the 35 real documents are fully readable (644 rows), 9 partially, 18 unrecognised, 3 are scans awaiting a vision provider — see design doc §5b
